...
# Add in extra information: the maxumum number of exons in a gene, so that can be later compared to the number of clean-intron-deletions identified,
# to aid in deciding if the retrocopied gene is a true-positive or false-positive.
# This script is also an opportunity to carry out any adjustments to data (in this case, fixing any erroneous VAFs).
...
function fix_depth_pair(depth_pair) {
  if ((depth_pair == ".") || (depth_pair == "")) {
    ...
    if (num_bits >= 2) {
      ...
      # In data created by older scripts, there is sometimes a bug in the input data. Sometimes the comma between the two numbers is missing.
...
BEGIN {FS="\t";OFS="\t"}
{
  ...
  if (in_bam != ".") {
    ...
    # Sometimes the older script that processed Gridss 1.4.1 BND records to identify VAF (variant allele frequency) of a deletion
    # has erroneously set VAF to ~1 when VAF is actually ~0.5. Thus, if possible, for each end of the deletion,
    # obtain bam file depths from either side of the BND point, then derive a new VAF from those bam sequencing depths.
