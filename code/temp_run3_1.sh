...
for infile in "${outdir}"/A020029517111.clean_intron_deletions_with_strand_and_vaf_and_depth_and_numExons_temp.tsv; do
  ...
  outfile="${outdir}"/"${sample}".insertion_points_for_deletions_that_are_retrocopied_genes_1_temp.tsv
  ...
  ./temp_run3_2.sh $genome $cohort $sample $outdir $tmpdir $infile_from_gridss $python_path $in_bam $infile $outfile
