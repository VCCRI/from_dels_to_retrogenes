...
for infile in "${outdir}"/*.clean_intron_deletions_with_strand_and_vaf.tsv; do
  ...
  echo './run_2b_add_depth_adjust_vaf_in_clean_intron_dels.sh' $genome $cohort $sample $outdir $tmpdir $in_bam $infile $outfile
  ./run_2b_add_depth_adjust_vaf_in_clean_intron_dels.sh $genome $cohort $sample $outdir $tmpdir $in_bam $infile $outfile
