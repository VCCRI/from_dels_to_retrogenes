...
while [[ $keep_looking_for_more_insertion_points == "1" ]]; do
  ...
  echo 'cut -d$''\t' '-f9-11' $prev_outfile '| grep -v' '^insertion' '| sort -k1,1V -k2,2V -k3,3V | grep -P -v' '^\t' '>' $blacklist_regions
  cut -d$'\t' -f9-11 $prev_outfile | grep -v '^insertion' | sort -k1,1V -k2,2V -k3,3V | grep -P -v '^\t' > $blacklist_regions
  echo ''
  ...
  echo 'cat' $tmp_outfile '| awk' 'BEGIN {FS="\t";OFS="\t"} {if (($9 != "") && (NR>1)) {print $0}}' '| cat' $prev_outfile '- >' $outfile
  cat $tmp_outfile | awk 'BEGIN {FS="\t";OFS="\t"} {if (($9 != "") && (NR>1)) {print $0}}' | cat $prev_outfile - > $outfile
  echo ''
  ...
  echo 'num_found=$(cat' $tmp_outfile '| awk' 'BEGIN {FS="\t";OFS="\t"} {if (($9 != "") && (NR>1)) {print $0}}' '| wc -l | cut -d'':' '-f1)'
  num_found=$(cat $tmp_outfile | awk 'BEGIN {FS="\t";OFS="\t"} {if (($9 != "") && (NR>1)) {print $0}}' | wc -l | cut -d':' -f1)
