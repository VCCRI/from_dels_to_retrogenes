...
for infile in "${outdir}"/A020029517111.clean_intron_deletions_with_strand_and_vaf_and_depth_and_numExons_temp.tsv; do

  infile_basename=$(basename $infile)
  IFS='.' read -r -a array <<< "$infile_basename"
  sample="${array[0]}"

  prev_outfile="${outdir}"/"${sample}".insertion_points_for_deletions_that_are_retrocopied_genes_1_temp.tsv
  outfile="${outdir}"/"${sample}".insertion_points_for_deletions_that_are_retrocopied_genes_N_temp.tsv
  ...
  ./temp_run5_2.sh $genome $cohort $sample $outdir $tmpdir $infile_from_gridss $python_path $in_bam $infile $prev_outfile $outfile
