#!/usr/bin/python
# python2 add_tranche2_and_filter_gridss_vcf.py [-max_BND_length_for_no_split_reads]
...
__copyright__ = 'Copyright 2020, Victor Chang Cardiac Research Institute'
# Note that this program was copied and slightly modified from a public domain program by the same author at Garvan Institute of Medical Research
...
######################################################
def filter_vcf_record_and_calc_new_fields( max_BND_length_for_no_split_reads, this_chrom, this_pos, this_alts, this_qual, this_filter, this_info_fields, vaf_version ):
	...
	if (is_homologous_to_reference == False):
		...
		# Calculating the VAF for a SV had traditionally been done by counting the split reads and read pairs (SR + RP) supporting the variant. 
		...
		if ("REF" in this_info_fields):
			if (is_integer(this_info_fields["REF"])):
				this_REF = int(this_info_fields["REF"])
		...
		if ("VF" in this_info_fields):
			if (is_float(this_info_fields["VF"])):
				this_VF = float(this_info_fields["VF"])
		...
		# If FILTER contains any of the following PASS, SINGLE_ASSEMBLY, ASSEMBLY_ONLY, NO_ASSEMBLY, LOW_QUAL, then don't automatically filter it out. It might be a true positive.
		# For these BND calls, if SRQ > 1 (Quality score of split reads supporting breakpoint), then assume it is a true positive.
		# If SRQ = 0, then see whether this BND would be expected to have any split-reads. If not, then assume it is a true positive.
		# Only if the SINGLE_ASSEMBLY/ASSEMBLY_ONLY/NO_ASSEMBLY/LOW_QUAL BND call fails all that, then assume it is a false positive and filter it out.
		# If (abs(POS - ALT) + length_of_insert_in_ALT_field) <= 100, 
		...
		# so a small SV of upto 100 length can be contained within a read without there being any split-reads and thus without there being any high quality split reads. 
			...
			list_of_possibly_good_filter_values = ( "PASS", "SINGLE_ASSEMBLY", "ASSEMBLY_ONLY", "NO_ASSEMBLY", "LOW_QUAL" )
			...
			if (filter_is_in_list_of_possibly_good_filter_values):
				...
				if (this_SRQ >= 1):
					...
					for this_alt in this_alt_fields:
						...
						if ( (len(this_alt) > 1) and (this_alt.find('.') > -1) ):
							# There is a dot before or after the nucleotides.
							# VCF spec section 5.4.9 says that a breakpoint was found at this location but the partner location could not be unambiguously determined.
							keep_this_vcf = False
						else:
	...
	if (keep_this_vcf):
		...
		new_BNDVAF = 0
		new_BNDVAF_denominator1 = this_REFPAIR + this_SR + this_RP + this_IC + this_AS
		new_BNDVAF_denominator2 = this_REF
		new_BNDVAF_denominator = max(new_BNDVAF_denominator1, new_BNDVAF_denominator2)
		#print(str(new_BNDVAF_denominator1) + " " + str(new_BNDVAF_denominator2) + " " + str(new_BNDVAF_denominator))
		if (new_BNDVAF_denominator > 0):
			new_BNDVAF = float(this_SR + this_RP + this_IC + this_AS) / float(new_BNDVAF_denominator)
			#print(str(this_REFPAIR) + " " + str(this_SR) + " " + str(this_RP) + " " + str(this_IC) + " " + str(this_AS) + " " + str(new_BNDVAF_denominator) + " " + str(new_BNDVAF))
		if (vaf_version == '2'):
			new_BNDVAF = 0
			new_BNDVAF_denominator = this_VF + this_REF + this_REFPAIR
			if (new_BNDVAF_denominator > 0):
				new_BNDVAF = float(this_VF) / float(this_VF + this_REF + this_REFPAIR)
...
def main():
	...
	parser.add_argument('-vaf_version', action="store", dest="vaf_version", required=False, help='If not present then vaf = (SR + RP + IC + AS) / (REFPAIR + SR + RP + IC + AS). If present and equals 2 then vaf = VF / (VF + REF + REFPAIR).')
	...
	vaf_version = '1'
	if (args.vaf_version is not None):
		vaf_version = str(args.vaf_version)
	...
	for inline in sys.stdin:
		...
		if (in_header == False): # We are processing VCF data records. We are no longer in the header part of the file.
			...
			keep_this_vcf, new_tranche, new_tranche2, new_BNDVAF = filter_vcf_record_and_calc_new_fields( max_BND_length_for_no_split_reads, this_chrom, this_pos, this_alts, this_qual, this_filter, this_info_fields, vaf_version )
